...
# ╔═╡ ebd2cca0-4129-11ef-04eb-6bc78745bd2f
begin
	using MethodOfLines
	using DomainSets
...
# ╔═╡ 70e331ec-b82d-41d6-ba1d-1249fdfc5e3b
@parameters t x

# ╔═╡ 67cccddd-fbe0-4c59-8e40-3643e7766ffc
@variables u(..);

# ╔═╡ b22bd27e-40a5-484e-9197-513de9f7450a
Dt = Differential(t)

# ╔═╡ 2ad973f4-150d-4cdc-bf09-53c97562b952
Dxx = Differential(x)^2

# ╔═╡ 2136d836-b6f1-4b5d-8124-df85c791e060
D = 2.33 * 10^-3 # mm^2/s

# ╔═╡ e3bfa054-8f4b-4add-8caa-56ae1469f43e
eq = Dt(u(t, x)) ~ D * Dxx(u(t, x))

# ╔═╡ fa01b993-6e29-44dc-a8e4-b7000b664811
function boxcar(x)
	value = IfElse.ifelse((x < -2.5) | (x > 2.5), 0.0, 1.0)
	return value
end

# ╔═╡ cf353cc9-40bc-4430-a63a-766885ceb834
bcs = [
	u(0, x) ~ boxcar.(x)
]

# ╔═╡ 614cdc48-4b6d-4f90-8e97-017a4792212f


# ╔═╡ 9864da52-6220-4a52-b635-9e42fc93daad


# ╔═╡ 42afa131-2ec0-487c-8a81-6b1d6f7b0373
xx = range(start=-5, stop=5, length=100)

# ╔═╡ 9a2e1244-1f47-472f-b11c-7abb24216310
plot(xx, boxcar.(xx), label="boxcar")

# ╔═╡ f3bde17c-b845-4ca6-b539-2c47c2d8c8e4
domains = [
	t ∈ Interval(0, 10000),
	x ∈ Interval(-10, 10)
]

# ╔═╡ 7464974a-29be-43da-91f5-940445ab6a3f
@named pdesys = PDESystem(eq, bcs, domains, [t, x], [u(t, x)])

# ╔═╡ 200e4c98-76a1-4b80-90dd-3289b76083ea
begin
...
# ╔═╡ a43adbc0-0882-41b9-949a-19b8f609f2c3
begin
...
# ╔═╡ 27f6d858-92cd-4dcc-843c-cc84f31d60a2
begin
	fracs = [0.0, 0.1, 0.33, 0.5, 0.8, 1.0]
	plt = plot()
	for frac in fracs
		index = maximum([trunc(Int, size(solu)[1] * frac), 1])
		plot!(discrete_x, solu[index, :], label="frac = $frac")
	end
	#plot!(discrete_x, solu[size(solu)[1]÷2, :], label="mid")
	#plot!(discrete_x, solu[begin, :], label="start")
	plt
end


# ╔═╡ 4a765c65-b815-449e-a476-18cb0d5f721f
size(discrete_t)

# ╔═╡ 82f768b2-5f7f-4dda-bc90-91ba28fdafc9
getstep(x) = abs(x[1] - x[2])

# ╔═╡ 19df7a7b-d895-4254-b500-b9c0ea16f6d0
begin
	Δx = getstep(discrete_x)
	integrals = Vector{Float64}(undef, 0)
	for ts in eachindex(discrete_t)
		push!(integrals, sum(Δx .* (solu[ts, :] .* solu[begin, :])))
	end
end

# ╔═╡ d765efb5-e24c-4093-b8d6-10c62f60774b
sum(Δx .* solu[end, :])

# ╔═╡ 851a5ad3-be9f-478f-b32f-b42fc0ef6dba
minimum(integrals)

# ╔═╡ 17edf1d4-e327-4663-bf11-55a6f8d72e73
begin
	plot(integrals ./ 5)
	xlabel!("timestep index")
	ylabel!("excited molecular fraction")
	ylims!(0., 1.0)
end

# ╔═╡ 481d565a-054c-4ef3-8011-f80ceb0078fb
getstep(discrete_t)

# ╔═╡ 1832656c-09f9-4da9-82b8-082e00817433
maximum(discrete_t)

# ╔═╡ 8c06e3d1-88eb-4002-90a3-03723689b864
a = []

# ╔═╡ 711e21c9-476f-4d7e-ba2e-3db55bc50e1d
typeof(a)

# ╔═╡ 6c38e4c9-5c69-40b9-bf66-f58cafd9c7e1
push!(a, 2//5)

# ╔═╡ 8202903b-ca95-42a7-8e49-e50a74ee4d45
a isa Vector

# ╔═╡ 2951938c-f515-4268-a5b5-89bbd343698f
a[end]
...
manifest_format = "2.0"
project_hash = "8b3ed71bf007c912a0171e42075103c0436f14df"
...
git-tree-sha1 = "92965d5e08827bb24f06e8a3eeccf150f56f29d8"
...
git-tree-sha1 = "76bc720a1591150acf95deb8a58345c3e1e256d9"
